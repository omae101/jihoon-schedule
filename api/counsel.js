...
- 중1: 보통 공통수학1(공수1)을 진행. 여유가 되면 파이썬(코딩)이나 중국어도 미리 익혀두면 좋다(SW·제2외국어 소양, 자유학기 활용).
- 영문법: 한 번으로 끝내지 말고 2~3회독으로 반복 숙달.
- 국어 문법: 중1부터 '틈틈히' 꾸준히, 중3 때는 고등 국어 문법까지 한 번은 자세히 1회독해 둔다(수능 국어의 토대).
- 중3: 통합사회·통합과학을 미리 봐두면 고교 진입(2028 내신 비중 큼)에 도움.
- 고1 대비(미리): 미적분·대수·공통수학1,2·확률과통계까지 봐둔다.
- 이과 지망: 화학·물리를 미리 돌려둔다.
- 수학(가장 중요): 심화가 반드시 제대로 이루어지고, 수학·과학 선행이 잘 되어 있어야 고등에서 1등급이 나온다. 그래서 중학교 때 아이 수준에 따라 올림피아드 수준의 어려운 문제를 풀어보는 경험, 심화 문제를 붙들고 '스스로 생각하는 시간'을 갖는 것이 매우 중요하다(빨리 푸는 것보다 깊이 사고하는 훈련). 단, 아이가 감당 가능한 선에서 — 좌절만 주는 과도한 난이도는 역효과.
  ...
  try {
    ...
    const payload = {
      ...
      system:
        ...
        "성향 정보에 '일주(일간+일지 자리 십성)'와 '신살'이 포함될 수 있습니다. 명리학적으로 조금 더 깊이 해석하되 반드시 학습·기질 조언으로 연결하세요: 일지 자리 십성(비겁=주관·독립, 식상=표현·재능, 재성=현실감각·목표지향, 관성=책임감·규율, 인성=수용·사고력)으로 공부 방식을 개인화하고, 신살은 공부와 연결해 해석하세요(문창귀인·학당귀인=학문·글재주·배움의 소질, 천을귀인=좋은 스승/조력자 복, 도화살=매력·표현력이나 집중 분산 주의, 역마살=활동·이동형 학습이 유리, 화개살=몰입·연구형 깊이). 이 명리 해석을 MBTI 학습 스타일과 '함께' 엮어 한 아이의 통합된 상으로 설명하세요. 단, 사주는 '참고용 경향'임을 은은히 유지하고 미신적 단정이나 불안 조장은 금지합니다. " +
