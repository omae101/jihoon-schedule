...
[구체 선행 로드맵 — 상위권·성실형 목표 예시 (이 앱 운영자 제공, 아이 성향·현행 성취도에 맞춰 강도 조절. 절대적 진도표가 아니라 방향)]
- 초3: 분수와 사칙연산을 '빠르고 정확하게'. 사칙연산이 어긋나지 않도록(정확도) + 빨리빨리(속도) 동시에 다진다 — 이후 모든 수학의 뿌리.
- 초5: 수학 '심화'를 제대로(단순 진도보다 심화문제로 사고력). 여기가 중등 수학의 갈림길.
- 초5~6: 초등 전 과정 '마무리'(구멍 없이 매듭). + 한자 자격증 이수(문해력 기반) + 독서 많이(어휘·독해력이 중등 성적을 가름).
- 중1: 여유가 되면 파이썬(코딩)이나 중국어를 미리 익혀두면 좋다(SW·제2외국어 소양, 자유학기 활용).
- 영문법: 한 번으로 끝내지 말고 2~3회독으로 반복 숙달.
- 중3: 통합사회·통합과학을 미리 봐두면 고교 진입(2028 내신 비중 큼)에 도움.
- 고1 대비(미리): 미적분·대수·공통수학1,2·확률과통계까지 봐둔다.
- 이과 지망: 화학·물리를 미리 돌려둔다.
- ※ 주의: 위는 '상위권 지향+성실형' 기준의 상한선. 아이가 버거워하면 현행 심화·정확도부터. 이해 없는 과속 선행은 역효과 — 항상 현행의 구멍 메우기가 우선.
`;
  ...
  try {
    ...
    const payload = {
      ...
      system:
        ...
        "성향 정보에는 사주 오행 중 '상대적으로 옅은 기운', 음양 균형, 올해 삼재 여부가 포함될 수 있습니다. 이를 활용해 '이 아이가 지금 채우면 좋은 것'(약한 영역을 어떤 활동·공부·습관으로 자연스럽게 보완할지)을 아이 강점을 살리는 방향으로 1~2가지 구체적으로 제안하세요. 삼재가 언급되면 절대 겁주지 말고 '기본·건강·꾸준함을 챙기는 해'로 가볍게 참고만 하세요. " +
